...
def initial_phonebook(data):
    print(data['PhoneBook'])
    Initial_PhoneBook = data['PhoneBook']
    rows, cols = len(data['PhoneBook']),5
    ...
    phone_book = []
    print(phone_book)
    phone_book.extend(Initial_PhoneBook)
    print(phone_book)
    return phone_book
...
ch = 1
# Opening JSON file
with open('phonebook.json',encoding='utf-8') as data_file:    
    data = json.load(data_file)
pb = initial_phonebook(data)
print("initial_pb: ",pb)

while ch in (1, 2, 3, 4, 5):
	ch = menu()
	if ch == 1:
		print("Inside add contact option: ", pb)
		if pb == None:
		    pb = []
		else:
		    pass
		pb = add_contact(pb)
		print("PB after adding contact: ",pb)
	elif ch == 2:
		pb = remove_existing(pb)
	elif ch == 3:
		pb = delete_all(pb)
	elif ch == 4:
	    print("pb in option 4: ",pb)
	    if pb == None:
